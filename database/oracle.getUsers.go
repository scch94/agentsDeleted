...
const (
	oracleGetUsers = "SELECT ua.oid, ua.user_id, cu.client_oid FROM user_adm ua FULL JOIN client_user cu ON ua.oid = cu.user_oid WHERE cu.client_oid = :1 AND cu.tenant_oid= :2"
)

func GetUsersOracle(ctx context.Context, agent *modelUtils.Agents) ([]modeldb.UsersDb, error) {
	...
	ins_log.Tracef(ctx, "this is the QUERY: %s and the params: agentID=%s, and tenant_oid=%v", oracleGetUsers, agent.AgentOid, config.Config.Tenant)

	db := GetDb()

	rows, err := db.QueryContext(ctx, oracleGetUsers, agent.AgentOid, config.Config.Tenant)
	if err != nil {
	...
	for rows.Next() {
		...
		err = isUserParentOracle(ctx, &user)
		if err != nil {
